...
function validateGroqShape(obj: any): obj is Omit<GroqExplanation, "provider" | "model" | "generated_at"> {
  ...
  if (!Array.isArray(obj.key_findings) || !obj.key_findings.every((f: any) => typeof f === "string")) return false;
  if (!Array.isArray(obj.limitations) || !obj.limitations.every((f: any) => typeof f === "string")) return false;
  return true;
}
...
Respond with ONLY a valid JSON object, no markdown, no code fences, matching exactly this shape:
{
  ...
  try {
    ...
    if (err?.name === "AbortError") {
      ...
      console.error(`[groq-explainer] Groq request failed: ${err?.message ?? err}`);
    }
    return null;
  }
}
