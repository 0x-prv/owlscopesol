...
export default function SearchBar({ initialValue = "", autoFocus = false, label, buttonLabel = "Analyze" }: { initialValue?: string; autoFocus?: boolean; label?: string; buttonLabel?: string }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="w-full">
      ...
      <div className="flex flex-col gap-3 rounded-lg border border-border bg-background p-3 shadow-sm transition duration-150 focus-within:border-accent focus-within:ring-2 focus-within:ring-accent/20 sm:flex-row sm:items-center">
        ...
        <button type="submit" disabled={busy} className="rounded-md bg-accent px-4 py-2 text-sm font-medium text-accent-foreground transition duration-150 hover:bg-accent/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent/30 disabled:cursor-not-allowed disabled:opacity-50">{busy ? "Searching" : buttonLabel}</button>
      </div>
