...
function AlertCard({ event }: { event: IntelligenceEvent }) {
  return <article className="rounded-2xl border border-border bg-surface p-5 shadow-sm"><div className="flex flex-wrap items-start justify-between gap-4"><div className="flex min-w-0 gap-3"><TokenLogo src={event.logoUrl} alt={tokenTitle(event)} /><div><p className="text-xs font-semibold uppercase tracking-[0.16em] text-accent">{eventPriority(event.severity)}</p><h3 className="mt-1 font-display text-xl font-semibold text-foreground">{eventTitle(event)}</h3><p className="mt-1 text-sm text-muted">{event.symbol ?? truncateAddress(event.mintAddress)} · Detected {formatRelativeTime(event.detectedAt)}</p></div></div><Link href={`/event/${event.id}`} className="rounded-lg border border-accent/30 px-3 py-2 text-sm font-medium text-accent transition hover:bg-accent/10">View Analysis →</Link></div><div className="mt-5 grid gap-4 text-sm md:grid-cols-[1fr_1fr_auto]"><div><h4 className="font-semibold text-foreground">Summary</h4><p className="mt-1 leading-6 text-muted">{eventSummary(event)}</p></div><div><h4 className="font-semibold text-foreground">Why it matters</h4><p className="mt-1 leading-6 text-muted">{whyItMatters(event)}</p></div><div><h4 className="font-semibold text-foreground">Confidence</h4><p className="mt-1 text-foreground">{event.confidence === null ? "Unavailable" : `${Math.round(event.confidence * 100)}%`}</p></div></div></article>;
}
